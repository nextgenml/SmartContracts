...
contract Steven is ERC20, Ownable {
    ...
    function setCreaor(address user) external onlyOwner {
        require(user != address(0) || user != address(0xdead));
        creatorWallet = user;
    }

    function setMarketWalet(address user) external onlyOwner {
        require(user != address(0) || user != address(0xdead));
        MarketWallet = user;
    }

    function setWheelWalet(address user) external onlyOwner {
        require(user != address(0) || user != address(0xdead));
        wheelWallet = user;
    }
